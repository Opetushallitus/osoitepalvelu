 ...
 * </b>Config.env["osoitepalvelu.localisations"]</b>
 *
 * See "index.html" how the pre-loading is done to following global variable:
 * <b>window.CONFIG.env["osoitepalvelu.localisations"]</b>
 *
 * @see index.html for preload implementation
 *
 * @author mlyly
 */
var localisation = angular.module('localisation', ['ngResource', 'config', 'I18n']);
...
localisation.factory('Localisations', ["$log", "$resource", "Config", function($log, $resource, Config) {
    ...
    return $resource(window.url('lokalisointi.resource'), {
        id: '@id'
    }, {
        update: {
            method: 'PUT',
...
localisation.service('LocalisationService', ["$log", "$q", "$http", "$interval", "Localisations",
    ...
    this.createMissingTranslations = function(key, locale, originalText, params) {
        ...
        angular.forEach(locales, function(l) {
            ...
            if (!self.hasTranslation(key, l)) {
                ...
                // Save this to server
                var newEntry = {category: "osoitepalvelu", key: key, locale: l, value: originalText};
    ...
    this.getTranslations = function() {
        Config.env["osoitepalvelu.localisations"] = Config.env["osoitepalvelu.localisations"] || [];
        return Config.env["osoitepalvelu.localisations"];
    };
    ...
    this.updateLookupMap = function() {
        ...
        for (var localisationIndex in Config.env["osoitepalvelu.localisations"]) {
            var localisation = Config.env["osoitepalvelu.localisations"][localisationIndex];
